...
export function AddMemberContent({ onConfirm, confirmLabel, minMembers = 1 }: AddMemberContentProps) {
  ...
  const updateLastMemberRole = useCallback((modelId: string, identityId: string | null) => {
    setSelected((prev) => {
      ...
      const lastIdx = prev.map((m, i) => m.modelId === modelId ? i : -1).filter((i) => i >= 0).pop();
  ...
  const getIdentityName = useCallback((identityId: string | null) => {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="flex-1 flex min-h-0">
        ...
        <div className="w-1/2 flex flex-col border-r border-border min-h-0">
          <div className="px-2 py-2 flex-shrink-0">
            <div className="flex items-center rounded-lg px-2 py-1.5" style={{ backgroundColor: "var(--secondary)" }}>
              <Search size={14} className="text-muted-foreground flex-shrink-0" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={t("providerEdit.searchModels")}
                className="ml-1.5 flex-1 text-[13px] text-foreground bg-transparent outline-none placeholder:text-muted-foreground/50"
              />
            </div>
          </div>
            ...
            {sections.length === 0 ? (
              ...
              sections.map((section) => (
                <div key={section.title}>
                  <div className="px-3 py-1 sticky top-0 z-10" style={{ backgroundColor: "var(--secondary)" }}>
                  ...
                  {section.data.map((model) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-1.5 text-left px-3 py-2.5 text-[13px] transition-colors ${
                          isActive
                      ...
                      >
                        ...
                        {isSelected && <span className="text-[11px] text-primary flex-shrink-0">✓</span>}
                      </button>
                    );
                  })}
                </div>
          ...
          {activeModel && lastActiveIdentityId !== undefined ? (
            <>
              <div className="px-3 py-2.5 flex-shrink-0 border-b border-border">
                <p className="text-[13px] font-semibold text-foreground truncate">{activeModel.displayName}</p>
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                >
                  <div className="h-6 w-6 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "var(--secondary)" }}>
                    <User size={12} className="text-muted-foreground" />
                  </div>
                  <span className="text-[13px] text-foreground flex-1 truncate">{t("chat.noIdentity")}</span>
                  {lastActiveIdentityId === null && <span className="text-xs text-primary font-semibold">✓</span>}
                </button>
                ...
                {identities.map((identity: Identity) => (
                  <button
                    ...
                    style={{ borderBottom: "0.5px solid var(--border)" }}
                  >
                    <div className="h-6 w-6 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "color-mix(in srgb, var(--primary) 15%, transparent)" }}>
                      <span className="text-[10px] font-bold text-primary">{identity.name.slice(0, 1)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] text-foreground truncate">{identity.name}</p>
                      {identity.systemPrompt && (
                        <p className="text-[11px] text-muted-foreground truncate">{identity.systemPrompt.length > 30 ? `${identity.systemPrompt.slice(0, 30)}…` : identity.systemPrompt}</p>
                      )}
                    </div>
                    {lastActiveIdentityId === identity.id && <span className="text-xs text-primary font-semibold">✓</span>}
                  </button>
                ))}
              </div>
            </>
            ...
            <div className="flex-1 flex items-center justify-center px-4">
              <p className="text-xs text-muted-foreground text-center">{t("chat.selectModel")}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-shrink-0 border-t border-border px-4 py-3" style={{ paddingBottom: "max(12px, env(safe-area-inset-bottom, 12px))" }}>
        {selected.length > 0 && (
            ...
            {selected.map((m, i) => {
              ...
              return (
                <span
                  ...
                  style={{ backgroundColor: "color-mix(in srgb, var(--primary) 10%, transparent)", color: "var(--foreground)" }}
                >
                  <span className="h-2 w-2 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
                  {modelName}{identityName ? `（${identityName}）` : ""}
                  <button
        ...
        >
          {confirmLabel ?? t("common.confirm")}{selected.length > 0 ? ` (${selected.length})` : ""}
        </button>
      </div>
    </div>
  );
...
export function AddMemberPicker({ open, onClose, onConfirm, minMembers, confirmLabel }: AddMemberPickerProps) {
  const { t } = useTranslation();

  const handleConfirm = useCallback((members: SelectedMember[]) => {
    onConfirm(members);
    onClose();
  }, [onConfirm, onClose]);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[75vh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-4 pt-4 pb-2">
          <DialogTitle className="text-base">{t("chat.addMember")}</DialogTitle>
        </DialogHeader>
        {open && <AddMemberContent onConfirm={handleConfirm} minMembers={minMembers} confirmLabel={confirmLabel} />}
      </DialogContent>
    </Dialog>
  );
}
