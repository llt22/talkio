...
export function ProviderEditPage({ editId, onClose }: { editId?: string; onClose?: () => void }) {
  ...
  const setProviderModelsEnabled = useProviderStore((s: ProviderStoreState) => s.setProviderModelsEnabled);
  ...
  const probeModelCapabilities = useProviderStore((s: ProviderStoreState) => s.probeModelCapabilities);

  const [selectedPreset, setSelectedPreset] = useState<string | null>(isEditing ? "__edit__" : null);
  ...
  const displayModels = savedProviderId
    ? getModelsByProvider(savedProviderId)
    : [];

  const filteredModels = modelSearch
    ? displayModels.filter((m: Model) =>
    ...
    try {
      if (savedProviderId) {
        ...
        updateProvider(savedProviderId, {
          ...
          type: providerType, customHeaders, enabled: providerEnabled,
        });
  ...
  }, [name, baseUrl, apiKey, providerType, customHeaders, providerEnabled, isEditing, savedProviderId, providers, testConnection, fetchModels, updateProvider, addProvider, addModelById, t]);
  ...
  const handleSave = useCallback(() => {
    ...
    onClose?.();
  }, [name, baseUrl, apiKey, providerType, customHeaders, providerEnabled, savedProviderId, updateProvider, addProvider, testPulledModels, disabledTestModels, addModelById, onClose]);

  const handleRefresh = async () => {
    if (!savedProviderId) return;
    setPulling(true);
    try { await fetchModels(savedProviderId); }
    catch { /* ignore */ }
    ...
    <div className="h-full overflow-y-auto" style={{ backgroundColor: "var(--secondary)" }}>
      <div className="max-w-lg mx-auto px-4 pt-6 pb-8">
        ...
        {!isEditing && !selectedPreset && (
          <>
            <p className="text-[15px] font-semibold text-foreground mb-4">{t("providerEdit.quickSelect")}</p>

            {/* OpenAI Compatible — hero card */}
            <button
              onClick={() => { setSelectedPreset("__custom__"); setProviderType("openai"); }}
              className="w-full mb-3 flex items-center rounded-2xl px-5 py-4 active:opacity-80"
              style={{ backgroundColor: "color-mix(in srgb, var(--primary) 5%, var(--card))", border: "2px solid color-mix(in srgb, var(--primary) 30%, transparent)" }}
            >
              <div className="mr-4 h-12 w-12 flex items-center justify-center rounded-xl" style={{ backgroundColor: "color-mix(in srgb, var(--primary) 10%, transparent)" }}>
                <IoLinkOutline size={24} color="var(--primary)" />
              </div>
              <div className="flex-1 text-left">
                <p className="text-[16px] font-bold text-foreground">{t("providerEdit.openaiCompatible")}</p>
                <p className="mt-0.5 text-[13px] text-muted-foreground">{t("providerEdit.openaiCompatibleHint")}</p>
              </div>
              <IoChevronForward size={20} color="var(--primary)" style={{ opacity: 0.5 }} />
            </button>

            <p className="mb-2 mt-2 px-1 text-[13px] text-muted-foreground/60">{t("providerEdit.directApi")}</p>
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(PROVIDER_PRESETS).map(([key, preset]) => (
                ...
                >
                  <svg width="24" height="24" viewBox="0 0 512 512" fill="var(--primary)"><path d={PRESET_SVG[key] ?? ""} /></svg>
                  <span className="mt-1.5 text-[14px] font-semibold text-foreground">{preset.name}</span>
                </button>
              ))}
            </div>
          </>
        ...
        {showForm && (
          <>
            ...
            {selectedPreset && !isCustom && !isEditing && (
              <button
                onClick={() => { setSelectedPreset(null); setConnected(null); }}
                className="w-full mb-4 flex items-center rounded-2xl px-4 py-3 active:opacity-80"
              ...
              >
                <svg width="24" height="24" viewBox="0 0 512 512" fill="var(--primary)"><path d={PRESET_SVG[selectedPreset] ?? ""} /></svg>
                  ...
                  {PROVIDER_TYPE_OPTIONS.map((opt) => (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: providerType === opt.value ? "color-mix(in srgb, var(--primary) 10%, transparent)" : "transparent",
                        color: providerType === opt.value ? "var(--primary)" : "var(--muted-foreground)",
                      }}
                    >
                      {opt.label}
                    </button>
            ...
            >
              <span className="text-[13px] font-medium text-muted-foreground">{t("providerEdit.advancedSettings")}</span>
              {showAdvanced ? <IoCaretUp size={16} color="var(--muted-foreground)" style={{ opacity: 0.5 }} /> : <IoCaretDown size={16} color="var(--muted-foreground)" style={{ opacity: 0.5 }} />}
            </button>

            {showAdvanced && (
              <div className="overflow-hidden rounded-xl mb-4" style={{ backgroundColor: "var(--card)" }}>
                <div className="flex items-center justify-between px-4 py-3.5" style={{ borderBottom: "0.5px solid var(--border)" }}>
                  <span className="text-[15px] text-foreground">{t("providerEdit.enabled")}</span>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" checked={providerEnabled} onChange={(e) => setProviderEnabled(e.target.checked)} className="sr-only peer" />
                    <div className="w-11 h-6 rounded-full peer-checked:bg-primary bg-muted-foreground/30 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full" />
                  </label>
                </div>
                <div className="px-4 py-3.5">
                  ...
                  {customHeaders.map((h: CustomHeader, idx: number) => (
                    <div key={idx} className="mb-2 flex items-center gap-2">
                      ...
                      />
                      <button onClick={() => setCustomHeaders(customHeaders.filter((_, i) => i !== idx))} className="p-1 active:opacity-60">
            ...
            <div className="mt-4 flex gap-3">
              ...
              >
                {testing || pulling ? (
                  ...
                  <span className="flex items-center gap-1.5"><IoCheckmarkCircle size={20} /> {t("providerEdit.connected")}</span>
                ) : connected === false ? (
            ...
            {savedProviderId && (
              <div className="mt-6">
                ...
                {/* Model Search */}
                <div className="mt-3 flex items-center rounded-xl px-3 py-2" style={{ backgroundColor: "var(--card)" }}>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0 mr-3">
                          <p className={`text-[15px] font-semibold truncate ${m.enabled ? "text-foreground" : "text-muted-foreground/40"}`}>
                            {m.displayName}
                          </p>
                          <p className="text-[12px] text-muted-foreground truncate">{m.modelId}</p>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer flex-shrink-0">
                          <input type="checkbox" checked={m.enabled} onChange={() => toggleModel(m.id)} className="sr-only peer" />
                          <div className="w-11 h-6 rounded-full peer-checked:bg-primary bg-muted-foreground/30 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full" />
                        </label>
                      </div>
                      <div className="mt-2 flex flex-wrap items-center gap-1.5">
                        {[
                          { key: "vision", label: t("providerEdit.vision"), on: m.capabilities?.vision, icon: <IoEyeOutline size={12} /> },
                          { key: "tools", label: t("providerEdit.tools"), on: m.capabilities?.toolCall, icon: <IoConstructOutline size={12} /> },
                          { key: "reasoning", label: t("providerEdit.reasoning"), on: m.capabilities?.reasoning, icon: <IoBulbOutline size={12} /> },
                        ].map((cap) => (
                          <span
                            key={cap.key}
                            className={`flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] ${!cap.on ? "opacity-30" : ""}`}
                            style={{ backgroundColor: "var(--muted)", color: cap.on ? "var(--foreground)" : "var(--muted-foreground)" }}
                          >
              ...
              {testPulledModels.map((m: { id: string; object: string }, idx: number) => {
                ...
                return (
                  <div
                    ...
                    style={{ borderBottom: idx < testPulledModels.length - 1 ? "0.5px solid var(--border)" : "none" }}
                  >
                    <div className="flex-1 min-w-0">
                      <p className={`text-[15px] font-semibold truncate ${enabled ? "text-foreground" : "text-muted-foreground"}`}>{m.id}</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={enabled}
                        onChange={() => setDisabledTestModels((prev) => {
        ...
        <div className="flex items-center justify-center gap-1.5 px-6 pt-10 pb-8">
          ...
          <span className="text-[11px] text-muted-foreground/40">{t("providerEdit.encryption")}</span>
        </div>
      </div>
    </div>
  );
...
function FormRow({ label, children, isLast = false }: { label: string; children: React.ReactNode; isLast?: boolean }) {
  return (
    <div
      className="flex items-center px-4 py-3.5"
      style={{ borderBottom: isLast ? "none" : "0.5px solid var(--border)" }}
    >
      <span className="w-24 text-[15px] text-foreground flex-shrink-0">{label}</span>
      {children}
    </div>
  );
}
